 ...
 */
public class MBBWPlayer3 implements PokerSquaresPlayer {
	 ...
	 */
	public MBBWPlayer3(int depthLimit) {
		for (int i = 0; i < SIZE; i++){
			for (int j = 0; j < SIZE; j++){
				visits[i][j] = 1;
			}
		...
		if (numPlays < 24) { // not the forced last play
			...
			for (int i = 0; i < remainingPlays; i++) { // for each legal play position
				...
				while (System.currentTimeMillis() < endTime || simCount == numIter) { // perform as many MC simulations as possible through the allotted time
					...
					epsilon = initEpsilon * (numIter - simCount) / numIter;
...
//		new PokerSquares(new MBBWPlayer3(), system).play(); // play a single game
//		System.out.println("******");
//		new PokerSquares(new MBBWPlayer2(), system).play(); // play a single game
		
		ArrayList<PokerSquaresPlayer> players = new ArrayList<PokerSquaresPlayer>();
		players.add(new MBBWPlayer3());
		players.add(new MBBWPlayer2());
